...
In order to send or receive messages to / from the Artemis Broker, you need to start a service.
You can either run Artemis in a container or from a downloaded Artemis binary distribution.

=== Run the Artemis container:

[source,shell]
----
docker run -d --rm -e AMQ_EXTRA_ARGS="--relax-jolokia" -e AMQ_USER=admin -e AMQ_PASSWORD=admin -p 5672:5672 -p 8161:8161 --name artemis quay.io/arkmq-org/activemq-artemis-broker:artemis.2.51.0
----
