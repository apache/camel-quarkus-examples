...
* `server-cert.pem` - Server certificate in PEM form, for clients that cannot read PKCS12 truststores (for example `curl --cacert`)
...
Client credentials are automatically generated during test execution in `target/certs/`:
...
To actually test X25519MLKEM768 hybrid key exchange, use a PQC-capable client. The test suite demonstrates this using RestAssured with a BouncyCastle JSSE-backed SSLContext:
