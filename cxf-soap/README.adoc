...
We will then communicate with the SOAP webservices (Camel consumer) directly via SOAP messages (ie. with `curl`).
...
```
{"contacts":[{"name":"Lukas","address":{"city":"Czech Republic","street":"Random Street"},"type":"OTHER"}]}
```
