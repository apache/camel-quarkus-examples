...
You don't need to provide a Keycloak instance when running in dev mode (a.k.a. `mvn quarkus:dev`) - read more https://quarkus.io/guides/getting-started#development-mode[here]
or when running the supplied tests (`mvn test`).
...
Use the same configuration as in <<external-keycloak-instance-configuration>> and obtain `QUARKUS_OIDC_CREDENTIALS_SECRET` and the Kubernetes base URL (BASE_KEYCLOAK_KUBERNETES_URL) to your keycloak instance.
